...
/**
 * Android input shell around the native C++ drawing engine.
 * Kotlin owns Android lifecycle/gestures; C++ owns brush sampling and rendering.
 */
class NativeCanvasView(context: Context) : GLSurfaceView(context) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                activeStroke = true
                nativeBeginStroke(nativeHandle, event.x, event.y, pressure(event, 0))
                return true
            }
            MotionEvent.ACTION_POINTER_DOWN -> {
                ...
                if (event.pointerCount >= 2) {
                    ...
                    lastSpanX = (event.getX(0) + event.getX(1)) / 2f
                    lastSpanY = (event.getY(0) + event.getY(1)) / 2f
                }
                nativeEndStroke(nativeHandle)
                return true
            }
            MotionEvent.ACTION_MOVE -> {
                if (event.pointerCount >= 2) {
                    ...
                    if (delta < -Math.PI) delta += (2 * Math.PI).toFloat()
                    rotation += delta
                    lastAngle = a; lastSpanX = cx; lastSpanY = cy
                    syncTransform()
                    return true
                }
                if (activeStroke) {
                    nativeMoveStroke(nativeHandle, event.x, event.y, pressure(event, 0))
                    lastX = event.x; lastY = event.y
                    return true
                }
            }
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                if (activeStroke) nativeEndStroke(nativeHandle)
                activeStroke = false
                performClick()
                return true
            }
    ...
    private fun pressure(event: MotionEvent, pointer: Int): Float = event.getPressure(pointer).coerceIn(.05f, 1.5f)
    private fun angle(e: MotionEvent): Float = atan2(e.getY(1) - e.getY(0), e.getX(1) - e.getX(0)).toFloat()
    @Suppress("unused") private fun distance(e: MotionEvent): Float { val dx=e.getX(1)-e.getX(0); val dy=e.getY(1)-e.getY(0); return sqrt(dx*dx+dy*dy) }
