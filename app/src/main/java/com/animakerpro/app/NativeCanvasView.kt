...
class NativeCanvasView(context: Context) : GLSurfaceView(context) {
    ...
    var taperStart = 0f; set(value){field=value.coerceIn(0f,1f);syncAdvanced()}
    var taperEnd = 0f; set(value){field=value.coerceIn(0f,1f);syncAdvanced()}
    ...
    var color = 0xFF111111.toInt(); set(value){field=value;if(nativeHandle!=0L)syncColor()}
    ...
    init{setEGLContextClientVersion(3);setRenderer(object:Renderer{
        override fun onSurfaceCreated(gl:javax.microedition.khronos.opengles.GL10?,config:javax.microedition.khronos.egl.EGLConfig?){nativeHandle=nativeCreate();syncBrush();syncAdvanced();syncColor();nativeSetEraser(nativeHandle,eraser);nativeSetOnionSkin(nativeHandle,onionSkin)}
